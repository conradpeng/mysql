...
- 复合索引
    ...
    - 匹配原则<br>
        ...
        1. 区分度越高越优先
            ...
            ```
            建立(a,b,c)或者(c,b,a)或者(b,a,c)都可以，重点是要让区分度高的字段放在前面，以保证后面的字段的局部匹配时有序的概率更高
...
- 聚簇索引优点
    ...
    1. `行数据与叶子节点存储在一起`，同一页（Innodb存储的最基本结构）会有多条行数据，访问统一数据页的不同行数据时，第一次访问便已经把该`页数据载入buffer`中了，再次访问时`会在内存中访问，不必访问磁盘`。主键和行数据一起被载入内存，找到叶子节点时，就立刻可以返回改行数据
